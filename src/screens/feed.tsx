...
// 2️⃣ 👇 PUT RANKING LOGIC RIGHT HERE
  ...
  return (
    <View style={{ flex: 1 }}>
      <FlatList
        data={rankedPosts}  // <-- 3️⃣ USE rankedPosts here
...
const handleWatchTime = (postId: string, seconds: number) => {
  updatePost(postId, {
    watchTime: (prevWatchTime || 0) + seconds,
  });
};
const PAGE_SIZE = 5;
const [page, setPage] = useState(1);

const paginatedPosts = rankedPosts.slice(0, page * PAGE_SIZE);
...
const verifiedBoost = (post: any) => {
  return post.user?.isVerified ? 10 : 0;
};

const calculateTimeDecay = (createdAt: any) => {
  const now = Date.now();
...
const calculateAIScore = (post: any) => {
  ...
  return [...filtered].sort((a, b) => {
    ...
    const scoreA = (rankA + trendingA + aiA + verifiedA) * decayA;
    const scoreB = (rankB + trendingB + aiB + verifiedB) * decayB;
...
import { PostItem } from "./PostItem"; // adjust path if needed
import { usePosts } from "./hooks/usePosts"; // adjust path
import { CATEGORIES } from "./constants"; // adjust path
import { styles } from "./styles"; // adjust path
...
export default function FeedScreen() {
  ...
  // -------------------------------
  const paginatedPosts = useMemo(
    () => boostedPosts.slice(0, page * pageSize),
    [boostedPosts, page]
  );
